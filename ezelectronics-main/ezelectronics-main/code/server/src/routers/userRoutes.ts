...
class UserRoutes {
    ...
    initRoutes() {
         ...
         */
        this.router.get(
            "/roles/:role", this.authService.isAdmin,
            check('role').isIn(['Customer', 'Manager', 'Admin']),
            this.errorHandler.validateRequest,
         ...
         */
        this.router.patch(
            "/:username", this.authService.isLoggedIn, check('username').isString().isLength({ min: 1 }),

            body("name").isString().isLength({ min: 1 }),
            body("surname").isString().isLength({ min: 1 }),
            body("address").isString().isLength({ min: 1 }),
            body('birthdate').isString().isLength({ min: 1 }).isDate({format: 'YYYY-MM-DD', strictMode: true}),
            this.errorHandler.validateRequest,
